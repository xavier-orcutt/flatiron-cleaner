...
class DataProcessorMelanoma(DataProcessorGeneral):
    ...
    T_STAGE_MAPPING = {
        ...
        'Tis': 'T0',
        'T0': 'T0',
        'TX (Not assessed)': 'unknown',
        'T Unknown/Not documented': 'unknown'
    }
    ...
    N_STAGE_MAPPING = {
        ...
        'N1c': 'N1',
        'N0': 'N0',
        'NX (Not assessed)': 'unknown',
        'N Unknown/Not documented': 'unknown'
    }

    M_STAGE_MAPPING = {
        'M1': 'M1',
        'M1a - Metastases to skin, subcutaneous or distant lymph nodes': 'M1',
        'M1c - Metastases to all other visceral sites or distant metastases to any site combined with an elevated serum LDH': 'M1',
        'M0': 'M0',
        'M Unknown': 'unknown'
    }
    ...
    RESIDUAL_DISEASE_MAPPING = {
        'R2 Macroscopic residual tumor ': 'R1',
        'R1/R2 Residual tumor, NOS': 'R1',
    ...
    def process_enhanced(self,
        ...
        """
        Processes Enhanced_AdvancedMelanoma.csv to standardize categories, consolidate 
        ...
        Notes
        -----
        Duration-based features not calculated for FirstLocalRecurDate, FirstDistantRecurDate, FirstVisceralMetDate, 
        ...
        try:
           ...
           # Select PatientIDs that are included in the index_date_df the merge on 'left'
            ...
            categorical_cols = ['TStage', 
                                ...
                                'DocumentedResectInitialDx',
                                'CalcResectInitialDx',
                                'ResidualDiseaseLocalRecur',
                                'DocumentedResectLocalRecur',
                                'CalcResectLocalRecur']
        
            df[categorical_cols] = df[categorical_cols].astype('category')

            # Recode categorical variables using class-level mapping and create new column
            ...
            logging.info(f"Successfully processed Enhanced_AdvancedMelanoma.csv file with final shape: {df.shape} and unique PatientIDs: {(df['PatientID'].nunique())}")
            self.enhanced_df = df
            return df

        except Exception as e:
            logging.error(f"Error processing Enhanced_AdvancedMelanoma.csv file: {e}")
            return None
