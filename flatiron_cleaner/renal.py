...
class DataProcessorRenal(DataProcessorGeneral):
    ...
    def process_biomarkers(self,
        ...
        """
        Processes Enhanced_MetRCCBiomarkers.csv by determining biomarker status for each patient within a specified time window relative to an index date. 

        Parameters
        ----------
        file_path : str
            Path to Enhanced_MetRCCBiomarkers.csv file
        index_date_df : pd.DataFrame
        ...
        try:
            df = pd.read_csv(file_path)
            logging.info(f"Successfully read Enhanced_MetRCCBiomarkers.csv file with shape: {df.shape} and unique PatientIDs: {(df['PatientID'].nunique())}")
            ...
            logging.info(f"Successfully merged Enhanced_MetRCCBiomarkers.csv df with index_date_df resulting in shape: {df.shape} and unique PatientIDs: {(df['PatientID'].nunique())}")
            ...
            logging.info(f"Successfully processed Enhanced_MetRCCBiomarkers.csv file with final shape: {final_df.shape} and unique PatientIDs: {(final_df['PatientID'].nunique())}")
            self.biomarkers_df = final_df
            return final_df

        except Exception as e:
            logging.error(f"Error processing Enhanced_MetRCCBiomarkers.csv file: {e}")
            return None
    ...
    def process_insurance(self,
        ...
        Final insurance indicators are set as follows:
        - medicare: Set to 1 for PayerCategory = Medicare
        - medicaid: Set to 1 for PayerCategory = Medicaid
        - commercial: Set to 1 for PayerCategory = Commercial Health Plan
        - other_insurance: Set to 1 for PayerCategory = Other Payer - Type Unknown, Other Government Program, Patient Assistance Program, Self Pay, or
    ...
    def process_mortality(self,
        ...
        biomarkers_path : str
            Path to Enhanced_MetRCCBiomarkers.csv file, using SpecimenCollectedDate to determine last EHR activity date for censored patients
        oral_path : str
            Path to Enhanced_MetRCC_Orals.csv file, using StartDate and EndDate to determine last EHR activity date for censored patients
        progression_path : str
            Path to Enhanced_MetRCCProgression.csv file, using ProgressionDate and LastClinicNoteDate to determine last EHR activity date for censored patients
        drop_dates : bool, default = True
